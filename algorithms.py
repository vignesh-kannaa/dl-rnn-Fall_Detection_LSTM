    ...
    while True:
        ...
        if not any(q.empty() for q in queues):
            ...
            if argss[0].num_cams == 1:
                ...
                r = requests.get('http://www.iotclouddata.com/22log/191/iot22.php?A='+activity_dict[prediction+5])
                print(r)
		...
		#print("hey")
                ...
                my_list = (activity_dict[prediction+5])
                print(my_list)
                if 'Normal' in my_list:
                    print("tested normal")
                    #Mail = input("Enter Required Mail Id : ")
                    mail_content = "normal"###
                    sender_address = 'divyagirija69@gmail.com'
                    sender_pass = 'divyagirija'
                    receiver_address = 'divyagirija69@gmail.com'
                    message = MIMEMultipart()
                    message['From'] = sender_address
                    message['To'] = receiver_address
                    message['Subject'] = 'NORMAL'   #The subject line
                    ...
                    print("tested none")
                    #ser.write(b'A')
                elif 'FALL Warning' in my_list:
                    print("tested FALL Warning")
                    mail_content = "fall warning"
                    sender_address = 'divyagirija69@gmail.com'
                    sender_pass = 'divyagirija'
                    receiver_address = 'divyagirija69@gmail.com'
                    message = MIMEMultipart()
                    message['From'] = sender_address
                    message['To'] = receiver_address
                    message['Subject'] = 'FALL WARNING'   #The subject line
                    message.attach(MIMEText(mail_content, 'plain'))
                    # session = smtplib.SMTP('smtp.gmail.com', 587) #use gmail with port
                    # session.starttls() #enable security
                    # session.login(sender_address, sender_pass) #login with mail_id and password
                    ...
                    print('Mail Sent')
                    #ser.write(b'A')
                elif 'FALL'in my_list:
                    print("tested FALL")
                    mail_content = "fall detected"
                    sender_address = 'divyagirija69@gmail.com'
                    sender_pass = 'divyagirija'
                    receiver_address = 'divyagirija69@gmail.com'
                    message = MIMEMultipart()
                    message['From'] = sender_address
                    message['To'] = receiver_address
                    message['Subject'] = 'FALL DETECTED'   #The subject line
                    message.attach(MIMEText(mail_content, 'plain'))
                    # session = smtplib.SMTP('smtp.gmail.com', 587) #use gmail with port
                    # session.starttls() #enable security
                    # session.login(sender_address, sender_pass) #login with mail_id and password
		...
		#print(get_all_features(ip_sets[0], lstm_sets[0], model))
		...
		#print(activity_dict[prediction2+5]) 
                r = requests.get('http://www.iotclouddata.com/22log/191/iot22.php?A='+activity_dict[prediction+5])
                print(r)  
